...
class Boosting:
    ...
    def fit_new_base_model(self, x, y, predictions):
        ind = np.random.choice(x.shape[0], int(self.subsample * x.shape[0]), replace=True)
        s = -self.loss_derivative(y[ind], predictions[ind])
        clf = self.base_model_class(**self.base_model_params)
        clf.fit(x[ind], s)

        gammas = self.find_optimal_gamma(y[ind], predictions[ind], clf.predict(x[ind]))
    ...
    def fit(self, x_train, y_train, x_valid, y_valid):
        ...
        if self.plot:
            # plt.plot(np.arange(len(self.history['train'])), self.history['train'])
            plt.plot(np.arange(len(self.history['validation'])), self.history['validation'])
            plt.title('valid loss на разных итерациях')
