...
class Adam(VanillaGradientDescent):
    ...
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        m_, v_ = self.m / (1- self.beta_1**(self.lr.iteration+1)), self.v / (1 - self.beta_2**(self.lr.iteration+1))
        step = np.divide(-self.lr()*m_, np.sqrt(v_) + self.eps)
        self.w += step     
        return step
...
class BaseDescentReg(BaseDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        l2_gradient[-1] = 0 #столбец с единичками последний в матрице признаков, свободный член не регуляризуется
